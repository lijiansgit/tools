...
var (
	listenAddress = kingpin.Flag(
		"web.listen-address",
		"Address to listen on for web interface and telemetry, must not 0.0.0.0:9104/:9104",
	).Default("127.0.0.1:9104").String()
	...
	prometheusConfPath = kingpin.Flag(
		"prometheus.conf-path",
		"Path prometheus config yaml path.",
	).Default("/data/app/prometheus/prometheus.yml").String()
...
func newHandler(metrics collector.Metrics, scrapers []collector.Scraper, dsn string) http.HandlerFunc {
...
func main() {
	...
	RDSList := GetRDSList()
	for k, RDSID := range RDSList {
		// write prometheus conf file
		if k == 0 {
			CreateFile()
		}
		WriteFile(RDSID)
		...
		dsn = fmt.Sprintf("%s:%s@(%s.mysql.rds.aliyuncs.com:3306)/", MySQLUSERNAME, MySQLPASSWORD, RDSID)
		handlerFunc := newHandler(collector.NewMetrics(), enabledScrapers, dsn)
		RDSMetricPath := fmt.Sprintf("/metrics/%s", RDSID)
		log.Infof("exporter url: http://%s%s", *listenAddress, RDSMetricPath)
		http.HandleFunc(RDSMetricPath, prometheus.InstrumentHandlerFunc("metrics", handlerFunc))
	}
